...
class DRAW:
    ...
    def _flush_pending_draws(self):
        """Scarica OGNI buffer CPU pendente (quad + le 4 forme arrotondate),
        nell'ordine in cui sono stati aperti. Usato da _use_batch() quando
        si cambia tipo di disegno e da tutte le *Batch (plurali, GPU
        instanced) e Draw{Texture,SpritesBatch,TextBatch,...} che devono
        garantire l'ordine di disegno rispetto ai buffer 'immediate'."""
        if self.rect_count > 0:
            self.Flush()
        self.FlushRounded()
        self._active_kind = None
